<template>
  ...
  >
    <swiper-slide v-for="(card, index) in reviews" :key="index">
      <div
        class="flex flex-col gap-[17px] md:gap-6 h-full bg-white p-[29.78px] shadow-cardShadow sm:p-10 rounded-3xl w-full select-none"
      >
        <div class="self-stretch flex flex-col gap-[11px] md:gap-4">
          ...
          <div class="w-full">
            ...
            <p
              class="text-xs md:text-base font-inter text-black-low-emp leading-[22px]"
            >
        ...
        <div class="flex gap-[18px] mt-auto shrink-0">
          ...
          <div class="flex flex-col gap-[2px] lg:gap-1 w-full">
            ...
            <p
              class="text-black-low-emp font-inter text-xs lg:text-sm leading-[15.6px] lg:leading-[18px] w-full whitespace-nowrap text-ellipsis"
            >
              {{ card?.reviewPosition }}
            </p>
          </div>
...
:root {
  --pagination-bullet-color: #eff1fe;
}
