<template>
  ...
  >
    <swiper-slide v-for="(card, index) in reviews" :key="index">
       <div class=" flex flex-col gap-[17px] md:gap-6 bg-white p-10 rounded-3xl w-full mb-8 h-[320px]">
        <div class="self-stretch flex flex-col gap-4">
          <div class="pb-[11px] lg:pb-4">
            <svg
          ...
          <div class="w-full ">
            ...
            <p class="text-base font-inter text-black-low-emp leading-[22px]">
              {{ truncateReview(card?.review, 100) }}
            </p>
          </div>
        </div>

        <div class="flex gap-[18px] mt-auto shrink-0">
          <div class="w-[50px] h-[50px] rounded-[100px]">
            <img
              :src="card?.img"
              alt=""
              class="w-9 h-9 lg:w-[50px] lg:h-[50px] rounded-[100px] object-cover"
            />
          </div>
          <div class="flex flex-col gap-[2px] lg:gap-1 w-full">
            <p
              class="text-primary-color-900 font-inter text-sm leading-[19.6px] lg:text-xl lg:leading-7 uppercase lg:capitalize"
            >
              {{ card?.reviewName }}
            </p>
            <p
              class="text-black-low-emp font-inter text-xs lg:text-sm leading-[15.6px] lg:leading-[18px] w-full"
            >
              {{ card?.reviewPosition }}
            </p>
          </div>
